...
{
    public class GluesController : ApiControllerBase
    {
        private readonly IGluesService _service;

        public GluesController(IGluesService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<ActionResult> AddAsync([FromBody] GlueDto model)
        {
            return StatusCodeResult(await _service.AddAsync(model));
        }

        [HttpPost]
        public async Task<ActionResult> UpdateAsync([FromBody] GlueDto model)
        {
