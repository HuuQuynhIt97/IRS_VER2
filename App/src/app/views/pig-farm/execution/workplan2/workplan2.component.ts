...
@Component({
  ...
  templateUrl: './workplan2.component.html',
  styleUrls: ['./workplan2.component.scss']
})
export class Workplan2Component implements OnInit {
  ...
  uploadFile() {
    ...
    this.service.importExcelWorkPlan2(this.file, this.datePipe.transform(this.workPlanDate, 'yyyy-MM-dd'))
    .subscribe((res: any) => {
      // if(res.status) {
  ...
  getAll() {
    this.service.getAllWorkPlan2().subscribe((res: any) => {
      this.data = res.result;
      this.time_upload = res.time_upload;
    });
  }
