import { InChemical } from './../../../../_core/_model/stock/inChemical';
import { ChemicalService } from './../../../../_core/_service/setting/chemical.service';
import { InChemicalService } from './../../../../_core/_service/stock/inChemical.service';
...
@Component({
  selector: 'app-in-chemical',
  templateUrl: './in-chemical.component.html',
  styleUrls: ['./in-chemical.component.scss'],
  providers: [
    DatePipe
  ]
})
export class InChemicalComponent extends BaseComponent implements OnInit, OnDestroy {
  ...
  endDate: Date;
  buildings: any;
  model: InChemical = {} as InChemical;
  public editSettings = {
  ...
  constructor(
    ...
    private service: InChemicalService,
  ...
  finishOutOfStock(data: InChemical) {
  ...
  update(modelSup) {
    this.alertify.confirm4(
      ...
      this.alert.updateMessage,
      () => {
        this.service.update(modelSup as InChemical).subscribe(
          (res) => {
            if (res.success === true) {
              this.alertify.success(this.alert.updated_ok_msg);
              this.getInk();
