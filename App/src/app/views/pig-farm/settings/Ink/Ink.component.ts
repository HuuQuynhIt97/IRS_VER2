...
export class InkComponent extends BaseComponent implements OnInit, OnDestroy {
  @Input() height;
  ...
  // api
  getAudit(id) {
    this.serviceInk.getAudit(id).subscribe(data => {
      this.audit = data;
    });
  ...
  delete(id) {
    this.alertify.confirm4(
      ...
      () => {
        this.serviceInk.delete(id).subscribe(
          (res) => {
            if (res.success === true) {
  ...
  create() {
   this.alertify.confirm4(
      ...
      () => {
        this.serviceInk.add(this.model).subscribe(
          (res) => {
            if (res.success === true) {
  ...
  update() {
   this.alertify.confirm4(
      ...
      () => {
        this.serviceInk.update(this.model as Ink).subscribe(
          (res) => {
            if (res.success === true) {
  ...
  getById(id) {
   return this.serviceInk.getById(id).toPromise();
  }
 async openModal(template, data = {} as Ink) {
    this.loadHallData();
    this.getAllProcess()
    if (data?.id > 0) {
      ...
      this.title = 'INK_EDIT_MODAL';
    } else {
      this.model.id = 0;
      // this.model.startDate = new Date();
      // this.model.endDate = new Date(2099,11,31);
      this.title = 'INK_ADD_MODAL';
    }
